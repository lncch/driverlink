...
export default function ControlBar({ index, go, fullscreen, toggleFullscreen, toggleHelp }: Props) {
  return (
    <div className="bar">
      ...
      <button type="button" className="icon" onClick={toggleFullscreen} aria-label={fullscreen ? 'Leave fullscreen' : 'Enter fullscreen'}>
        {fullscreen ? '✕' : '⛶'}
      </button>
    </div>
  );
